...
#include "../simulation/position.h"
  ...
  GraphicContextIso::GraphicContextIso(Geography* map)
  {
    ...
    // Cette partie du code doit se retrouver ailleurs
  ...
  void GraphicContextIso::draw(sf::RenderTarget& target, sf::RenderStates states) const
  {
	...
	for(int j = std::min(w - 1, h+k-1); j >= std::max(k,0); j--)          // Vérifié par le calcul
	  {
	    int i = j - k;
	    Tile* tilec = map->getTile(i,j); // TODO : Changer le pointeur par une référence
  ...
  for(int k = w-1; k > -h; k--)
      ...
      for(int j = std::min(w - 1, h+k-1); j >= std::max(k,0); j--)          // Vérifié par le calcul
	{
	  ...
	  if(!tilec->TextureIsInit())
	    {
	      switch(tilec->getType())
		{
		case BANK:
		case HOUSE:
		  tilec->setTexture(&tilepackVector.at(1));
		  break;
		default:
		  tilec->setTexture(&tilepackVector.at(0));  // TODO : Faire une tile vide pour BLANK
		  break;
		}
	    }
