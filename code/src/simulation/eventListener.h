 ...
 */
template <class Child>
EventListener<Child>::EventListener(boost::uuids::uuid uuid) : GenericEventListener(uuid) {}
...
void EventListener<Child>::listen(EventName event, SourceT& source, void (Child::*callback)(SourceT&, ArgT&)) {
  ...
  auto run_callback = [callback,event](GenericEventListener* self, EventSource* sourcePtr, boost::any arg) { 
    if (arg.empty()) {
      std::cerr << "Event trigger typing error: the event was triggered without an argument yet the callback expects one.";
    } else {
      try {
        // We could die here if self does not actually point to a Child
        //                or if sourcePtr does not actually point to a SourceT
        auto closure = std::bind(callback, static_cast<Child*>(self), std::ref(*(static_cast<SourceT*>(sourcePtr))), boost::any_cast<reference<ArgT>>(arg));
        (static_cast<Child*>(self))->callbackReady(event, closure);
...
void EventListener<Child>::listen(EventName event, SourceT& source, void (Child::*callback)(SourceT&, ArgT)) {
  ...
  auto run_callback = [callback,event](GenericEventListener* self, EventSource* sourcePtr, boost::any arg) { 
    if (arg.empty()) {
      ...
      try {
        auto closure = std::bind(callback, static_cast<Child*>(self), std::ref(*(static_cast<SourceT*>(sourcePtr))), boost::any_cast<ArgT>(arg));
        (static_cast<Child*>(self))->callbackReady(event, closure);
      } catch (boost::bad_any_cast& e) {
        // Try to copy-construct a reference
        try {
          auto closure = std::bind(callback, static_cast<Child*>(self), std::ref(*(static_cast<SourceT*>(sourcePtr))), static_cast<ArgT>(boost::any_cast<reference<ArgT>>(arg)));
          (static_cast<Child*>(self))->callbackReady(event, closure);
        } catch (boost::bad_any_cast& e) {
          std::cerr << "Event trigger typing error: the event argument type and one of the callback types are incompatible" << std::endl;
        }
...
void EventListener<Child>::listen(EventName event, SourceT& source, void (Child::*callback)(SourceT&)) {
  ...
  auto run_callback = [callback,event](GenericEventListener* self, EventSource* sourcePtr, boost::any arg) { 
    std::bind(callback, static_cast<Child*>(self) ,std::ref(*(static_cast<SourceT*>(sourcePtr))))();
  };
